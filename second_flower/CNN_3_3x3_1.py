...
class CNN(nn.Module):
    def __init__(self, n_classes):
        ...
        self.conv2 = nn.Sequential(  # 下一个套餐的输入 (32, 112, 112)
            nn.Conv2d(32, 64, 3, 1, 1),  # 输出 (64, 112, 112)
            # nn.Conv2d(64, 64, 3, 1, 1),     # 输出 (64, 112, 112)
            nn.BatchNorm2d(64),
            nn.ReLU(),  # relu层
        ...
        self.conv3 = nn.Sequential(  # 下一个套餐的输入 (64, 56, 56)
            nn.Conv2d(64, 128, 3, 1, 1),  # 输出 (128, 56, 56)
            # nn.Conv2d(128, 128, 3, 1, 1),     # 输出 (128, 56, 56)
            nn.BatchNorm2d(128),
            nn.ReLU(),  # relu层
            nn.MaxPool2d(2),  # 输出 (128, 28, 28)
        )

        self.conv4 = nn.Sequential(  # 下一个套餐的输入 (128, 28, 28)
            nn.Conv2d(128, 256, 3, 1, 1),  # 输出 (256, 28, 28)
            # nn.Conv2d(256, 256, 3, 1, 1),     # 输出 (256, 28, 28)
            nn.BatchNorm2d(256),
            nn.ReLU(),  # relu层
            nn.MaxPool2d(2),  # 输出 (256, 14, 14)
        )

        self.conv5 = nn.Sequential(  # 下一个套餐的输入 (256, 14, 14)
